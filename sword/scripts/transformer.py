...
class Transformer:
    def __init__(self, pos_franka=None, rot_franka=None, load_from_file=True) -> None:
        self.client = OptiTrackClient()
        self.client.get_pose(0)
        print(';.')
        ...
        if load_from_file:
            ...
            self.pos1, rot1, self.markers1 = np.load(
                "pose1.npy", allow_pickle=True)
            self.pos2, rot2, self.markers2 = np.load(
                "pose2.npy", allow_pickle=True)
            print(f"pos1: {self.pos1}")
            print(f"pos2: {self.pos2}")
            print(f"rot2: {rot2}")
        else:
            pos1, rot1, markers1 = self.client.get_pose(0)
            self.markers1 = np.array(markers1)
            self.pos1 = np.array(pos1)
            rot1 = np.array(rot1)
            np.save("pose1.npy", [self.pos1, rot1,
                    self.markers1], allow_pickle=True)
            # head marker (2)left_eye (4)right_eye
            pos2, rot2, markers2 = self.client.get_pose(1)
            self.markers2 = np.array(markers2)
            self.pos2 = np.array(pos2)
            rot2 = np.array(rot2)
            self.markers2 = np.array(markers2)
            np.save("pose2.npy", [self.pos2, rot2,
                                  self.markers2], allow_pickle=True)
            print(f"pos1: {self.pos1}")
            print(f"pos2: {self.pos2}")
    ...
    def world_to_head2(self, sampling_count=10):
    ...
    def world_to_head(self, pos, rot):
        pos_in_head = pos - self.pos2
        rot_in_head = self.rot2.rot.inv() * R.from_quat(rot)
        relative_pos = pos_in_head / self.bbx + \
            self.center  # - np.array([0.5, 0.5, 0.5])

        return relative_pos, rot_in_head.as_quat()

    def head_to_world(self, pos, rot):
        rot_in_world = self.rot2.rot * R.from_quat(rot)
        pos_in_world = (pos + self.center) * self.bbx + self.pos2
        return pos_in_world, rot_in_world

    def franka_to_head_points(self, points):
        res = []
        for point in points:
            p, r = self.franka_to_world(point, [0, 0, 0, 1])
            p, r = self.world_to_head(p, r)
            res.append(p)
        return np.array(res)

    def franka_to_head(self, pos, rot):
        pos = np.array(pos)
        rot = np.array(rot)
        world_pos, world_rot = self.franka_to_world(pos, rot)
        # direction = R.from_quat(world_rot).apply(np.array([0, 1, 0]))
        # points_on_stick = np.array(
        #     [world_pos + 0.1 * i * direction for i in range(5)])
        # print(points_on_stick)
        # print("*************")
        return self.world_to_head(world_pos, world_rot)

    def head_to_franka(self, pos, rot):
        pos = np.array(pos)
        rot = np.array(rot)
        world_pos, world_rot = self.head_to_world(pos, rot)
        return self.world_to_franka(world_pos, world_rot)
